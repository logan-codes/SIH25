                                ...
                                <div className="text-center mb-8">
                                    {result.error ? (
                                        ...
                                        <div className={`inline-flex items-center px-6 py-3 rounded-full text-2xl font-bold ${
                                            result.isAuthentic ? 'bg-green-900 text-green-400 border border-green-500' : 'bg-red-900 text-red-400 border border-red-500'
                                        }`}>
                                            <i className={`fas ${result.isAuthentic ? 'fa-check-circle' : 'fa-times-circle'} mr-3`}></i>
                                            {result.isAuthentic ? 'Certificate Authentic' : 'Possible Forgery Detected'}
                                        </div>
                                    )}
                                    <p className="text-slate-400 mt-4">
                                        {result.error ? 'Error occurred during verification' : `Confidence Score: ${result.confidence.toFixed(1)}%`}
                                    </p>
                                    {result.error && (
                                ...
                                <div className="bg-slate-700 rounded-lg p-6 mb-6">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        {Object.entries(result.checks).map(([check, passed]) => (
                                            <div key={check} className="flex items-center justify-between py-2">
                                                <span className="text-slate-300 capitalize">
                                                    {check.replace(/([A-Z])/g, ' $1').trim()}
                                                </span>
                                                <i className={`fas ${passed ? 'fa-check-circle text-green-400' : 'fa-times-circle text-red-400'} text-lg`}></i>
                                            </div>
                                        ))}
                                    </div>
                                </div>
